var FILE_TYPES, FolderWatcher, GitManager,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

FILE_TYPES = require(__dirname + "/../file_types.js");

FolderWatcher = require(__dirname + "/../filestorage/folderwatcher.js");

GitManager = require(__dirname + "/gitmanager.js");
...
this.ProjectManager = (function() {
  ...
  ProjectManager.prototype.getGitManager = function() {
    if (this.git_manager == null) {
      this.git_manager = new GitManager(this.project);
    }
    return this.git_manager;
  };

  ProjectManager.prototype.startFolderWatcher = function() {
    if (!this.project.local_folder) {
      return;
    }
    if (this.folder_watcher == null) {
      this.folder_watcher = new FolderWatcher(this.project);
    }
    return this.folder_watcher.start();
  };

  ProjectManager.prototype.stopFolderWatcher = function() {
    if (this.folder_watcher != null) {
      this.folder_watcher.stop();
      return this.folder_watcher = null;
    }
  };
  ...
  ProjectManager.prototype.listFiles = function(folder, callback) {
    ...
    return this.project.getStorage().list(file, (function(_this) {
  ...
  ProjectManager.prototype.readProjectFile = function(session, data) {
    ...
    return this.project.getStorage().read(file, encoding, (function(_this) {
  ...
  ProjectManager.prototype.writeProjectFile = function(session, data) {
    var content, f, file, ref, remaining, storage, th, version;
    ...
    if ((data.content != null) && data.content.length > 40000000) {
      session.showError("File too large.");
      return;
    }
    if ((data.content != null) && data.content.length > 1000000) {
    ...
    if (data.content != null) {
      ...
      storage.write(file, content, (function(_this) {
        return function() {
          version += 1;
          ...
          if (data.properties != null) {
            ...
            if (typeof storage.writeProperties === "function") {
              storage.writeProperties(file, data.properties);
            }
          }
          if (data.request_id != null) {
            session.send({
    ...
    if (data.thumbnail != null) {
      ...
      f[3] = f[3].split(".")[0] + ".png";
      f = f.join("/");
      return storage.write(f, th, (function(_this) {
        return function() {
  ...
  ProjectManager.prototype.renameProjectFile = function(session, data) {
    ...
    storage = this.project.getStorage();
    return storage.read(source, "binary", (function(_this) {
      return function(content) {
        if (content != null) {
          return storage.write(dest, content, function() {
            var properties;
            properties = _this.getFileProperties(data.source);
            _this.setFileProperties(data.dest, properties);
            if (typeof storage.writeProperties === "function") {
              storage.writeProperties(dest, properties);
            }
            ...
            _this.project.touch();
            return storage["delete"](source, function() {
              if (data.thumbnail) {
                source = source.split("/");
                source[2] += "_th";
                ...
                dest = dest.join("/");
                return storage.read(source, "binary", function(thumbnail) {
                  if (thumbnail != null) {
                    return storage.write(dest, thumbnail, function() {
                      return storage["delete"](source, function() {
                        session.send({
  ...
  ProjectManager.prototype.deleteProjectFile = function(session, data) {
    ...
    file = this.project.owner.id + "/" + this.project.id + "/" + data.file;
    storage = this.project.getStorage();
    storage["delete"](file, (function(_this) {
      return function() {
    ...
    if (data.thumbnail) {
      ...
      f[3] = f[3].split(".")[0] + ".png";
      f = f.join("/");
      return storage["delete"](f, (function(_this) {
        return function() {};
      })(this));
    }
  };
          ...
          if (/^(ms|sprites|maps|sounds|music|doc|assets|sounds_th|music_th|assets_th)\/[a-z0-9_]{1,40}([-\/][a-z0-9_]{1,40}){0,10}.(ms|py|js|lua|png|json|wav|mp3|ogg|flac|md|glb|obj|jpg|ttf|txt|csv|wasm)$/.test(filename)) {
            ...
            try {
              return contents.file(filename).async(type).then((function(fileContent) {
                if (fileContent != null) {
                  return _this.project.getStorage().write(_this.project.owner.id + "/" + _this.project.id + "/" + dest, fileContent, funk);
                } else {
                  return funk();
                }
              }), function() {
  ...
  ProjectManager.prototype.syncFiles = function(session, data, source) {
    var dest_storage, funk, ops, source_storage, syncFile;
    ops = data.ops;
    if (!this.canWrite(session.user)) {
      return;
    }
    ...
    dest_storage = this.project.getStorage();
    source_storage = source.getStorage();
    syncFile = (function(_this) {
      return function(f, callback) {
        var file;
        file = source.owner.id + "/" + source.id + "/" + f;
        return source_storage.read(file, "binary", function(content) {
          var th;
          if (content != null) {
            file = _this.project.owner.id + "/" + _this.project.id + "/" + f;
            dest_storage.write(file, content, function() {
              if (callback != null) {
                return callback();
              }
            });
            if (f.startsWith("assets/") || f.startsWith("sounds/") || f.startsWith("music/")) {
              ...
              return source_storage.read(file, "binary", function(content) {
                if (content != null) {
                  file = _this.project.owner.id + "/" + _this.project.id + "/" + f;
                  return dest_storage.write(file, content, function() {});
                }
              });
            }
          } else {
          ...
          if (op.op === "sync") {
            ...
            return syncFile(f.path, function() {
              ...
              if (f.properties != null) {
                ...
                if (typeof dest_storage.writeProperties === "function") {
                  dest_storage.writeProperties(_this.project.owner.id + "/" + _this.project.id + "/" + f.path, f.properties);
                }
              }
              funk();
            ...
            return dest_storage["delete"](file, function() {
